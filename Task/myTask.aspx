<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="myTask.aspx.cs" Inherits="JiaoShiXinXiTongJi.Task.myTask" %>
    ...
    function dateformat(value){
        ...
        $(function () {
            ...
            { field: 'TaskName', title: '任务名称', width: 360, dataIndex: 'TaskID', align: 'left', formatter: function (value, rowData, rowindex) {
                ...
                var result = "<b><a href='TaskInfo.aspx?id=" + rowData.TaskID + "' title="+value+"  style='TEXT-DECORATION:none'>"+value+"</a></b>";
            ...
            { field: 'State_', title: '完成情况', width: 80, align: 'center', formatter: function (value, rowData, rowindex) {
                ...
                var result = "<font color=blue>" + value + "</font>";
                if (diff > 1) {
                ...
                parent.$.messager.confirm("系统提示", "是否确定删除任务【" + TaskName + "】的信息?", function(r) {
                    if (r) {
                        jQuery.ajax({
                            ...
                            data: "Action=myDelete&TaskID=" + TaskID+"&userid="+userid,
                            success: function(result) {
                                ...
                                parent.$.messager.alert("【" + TaskName + "】任务信息删除失败。");
                            }
                        });
                    }
                });
...
function searchrenyuan() {
             search = $("#search").val();
             grid.datagrid({ url: '../Data/Handler.ashx', queryParams: { Action: "myQuery", search: search, userid: userid} }); //grid刷新
